...
class Administrator extends CI_Controller {
    ...
    function tambah_keuangan(){
		...
		$date_c = date_create($this->input->post('tgl'));
        if (isset($_POST['submit'])){
    ...
    function tampil_data(){
		...
		$this->session->set_userdata('tgl_bulan', $vtanggal);
    ...
    function cetak_laporan(){
		...
		$vtanggal_=$this->input->get('vtanggal');
		$vtanggal = $this->session->userdata('tgl_bulan');
        $data['tampil_data']=$this->Model_app->tampil_data($vtanggal);
		$data['tampil_data1']=$this->Model_app->tampil_data1($vtanggal);
		$data['identitas']=$this->Model_app->tampil_indentitas('identitas');
